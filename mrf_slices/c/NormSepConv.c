 ...
 *
 *   B = NormSepConv(A,hx,hy,hz,mask)
 */
void mexFunction(int nlhs, mxArray *plhs[],
                 int nrhs, const mxArray *prhs[])
{
  if (nrhs != 5) {
      mexErrMsgIdAndTxt("NormSepConv:nrhs","Bad number of parameters");
  }
          
  if (!mxIsClass(prhs[0],"int16")) {
      mexErrMsgIdAndTxt("NormSepConv:datatype",
              "Input image must be of type int16");
  }
  
  if (!mxIsClass(prhs[1],"double") || !mxIsClass(prhs[2],"double") || !mxIsClass(prhs[3],"double")) {
      mexErrMsgIdAndTxt("NormSepConv:datatype","Kernels must be of type double");
  }
  if (NumberOfElements(prhs[1]) % 2 == 0 ||
      NumberOfElements(prhs[2]) % 2 == 0 ||
      NumberOfElements(prhs[3]) % 2 == 0)
  {
      mexErrMsgIdAndTxt("NormSepConv:kernelsize","Kernels must have an odd number of elements");
  }
  
  if (!mxIsClass(prhs[4],"logical") && !mxIsClass(prhs[4],"uint8")) {
      mexErrMsgIdAndTxt("NormSepConv:datatype","Mask must be of type logical/uint8");
  }

  if (!CheckSizesMatch(prhs[0],prhs[4])) {
      mexErrMsgIdAndTxt("NormSepConv:datatype","Mask and image must be of same dimensions");
  }
  
  plhs[0] = NormSepConv(prhs[0],prhs[1],prhs[2],prhs[3],prhs[4]);
}
