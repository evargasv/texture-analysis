...
% Copyright 2015 Toshiba Medical Visualization Systems Europe (TMVS)
    ...
    if ismember(1,mask_slice)
        
        % number of pixels on the mask
        px_mask = size(mask_slice(mask_slice(:) == 1),1);
        
        % the mask is big enough
        if px_mask > 16
            
            % extract statistics
            stats = regionprops(mask_slice,'BoundingBox');
            ...
            for i=1:nr_bound_bx
                ...
                bx_end_row = bx_init_row + bx_height;
                bx_end_col = bx_init_col+bx_width;

                % extract the mask portion (bounding box rectangle)
                mask = mask_slice(bx_init_row:bx_end_row,bx_init_col:bx_end_col);
